...
int main() {
    // // load database
    ifstream in("database_b.csv");
    string line;
    map<int, vector<double> > mapB;
    ...
    if (in.is_open()) {
        ...
        while (getline(in, line)) {
            ...
            int key = (int)v[0];
            v.erase(v.begin());
            mapB.insert(pair<int, vector<double> >(key, v));
        }     
    }
    in.close();      

    struct sockaddr_in server_B_addr, client_addr;
    ...
    if ((socket_B = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
    ...
    while (true) {
        ...
        recv_len = recvfrom(socket_B, &recv_buffer, (size_t)BUFFER_SIZE, 0, (struct sockaddr *)&client_addr, &client_addr_len);
        recv_buffer[BUFFER_SIZE] = '\0';
        if (recv_len > 0) {
            recv_buffer[recv_len] = '\0';
            extract_buffer(recv_buffer);
            cout << "The Server B received input <" << recv_buffer << ">" << endl;
            ...
            if (it == mapB.end()) {
                ...
                vector<double> list = mapB[link_id];
            ...
            cout << "The server B has found <" << count << "> match" << endl;



            // waiting AWS send to server A
            recvfrom(socket_B, &recv_buffer, (size_t)BUFFER_SIZE, 0, (struct sockaddr*)&client_addr, &client_addr_len);
            recv_buffer[BUFFER_SIZE] = '\0';
            // send result to AWS
